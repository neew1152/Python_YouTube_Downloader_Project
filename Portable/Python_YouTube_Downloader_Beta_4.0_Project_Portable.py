...
app.title("Python YouTube Video Downloader Beta 4.0 Project")
...
def search_format_function(url):
    ...
    try:
        ...
        # Update the graphical user interface to show the message
        app.update()
...
# Download function
def download_function(url, ID):
    # Include placeholders for the title and extension in the output template
    command = f"yt-dlp -f {ID} {url} -o Downloads/%(title)s.%(ext)s"
    try:
        # Clear any existing content in the frame
        monitor_command_output_frame.delete('1.0', tkinter.END)
        
        # Update the GUI to show the message
        app.update()
...
# Update GUI and Run app
app.update()
app.mainloop()

# The code is written by learning from
# youtu.be/NI9LXzo0UY0?si=1u8QGDkoxPAlTGEK
# youtu.be/Miydkti_QVE?si=10SWZsQkcRlB9lc3
# youtu.be/5aYwU4nj5QA?si=HqAzziu8WynyBy5m
# youtu.be/Envp9yHb2Ho?si=6MHc2Dz_6_2bt0G4
# youtu.be/EwL2BwEdduE?si=4mrdIAUNDDQTvj-f
# youtu.be/a0MVOloNLB4?si=xWd4W-Q8dpnm2rak
# youtu.be/GeflXCubyrA?si=ytOZIHvnd6J_87Vm
# youtu.be/QRroCuY1Bhc?si=ftS6KzHBPwhftw2k
